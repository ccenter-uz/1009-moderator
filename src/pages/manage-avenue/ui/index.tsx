...
export const ManageAvenuePage: FC = () => {
  ...
  const handleSubmit = async (values: valueProps) => {
    ...
    const request = editingData ? updateAvenue : createAvenue;
        ...
        if (record.status === STATUS.ACTIVE) {
          return (
            <Flex justify="center" align="center" gap={8}>
              ...
              />
              <DeleteTableItemUI fetch={() => deleteAvenue(record.id)} />
            </Flex>
          );
          ...
          return (
            <Tooltip title={t("restore")}>
              <MdRestore
                ...
                cursor={"pointer"}
                size={20}
                onClick={() => restoreAvenue(record.id)}
              />
            </Tooltip>
          );
        }
